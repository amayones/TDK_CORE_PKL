...
import { fetchTes2List, createTes2, updateTes2, deleteTes2 } from '../services/tes2Service';
import Tes2FormModal from '../components/Tes2FormModal';
...
export default function Tes2Page() {
    ...
    const loadItems = useCallback(async () => {
        setLoading(true);
        try {
            const data = await fetchTes2List();
            setItems(data);
        } finally {
            setLoading(false);
        }
    ...
    const handleSubmit = async (form) => {
        ...
        try {
            if (editingItem) {
                await updateTes2(editingItem.id, form);
                window.__APP__.alert('Data berhasil diupdate', 'success');
            } else {
                await createTes2(form);
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <div>
                    <h2 className="text-lg font-semibold text-gray-800">Tes2</h2>
                    <p className="text-gray-500 text-sm">Kelola data tes2.</p>
                </div>
                {hasPermission('tes2', 'can_create') && (
                    <button
