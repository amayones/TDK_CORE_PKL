...
Framework internal berbasis **Laravel 11 + React 18** untuk seluruh aplikasi perusahaan dengan arsitektur modular.
...
- Apache (atau server lain yang mendukung Laravel)

---

## 🚀 Setup Pertama Kali Setelah Clone

Karena beberapa file **tidak ikut di-commit** (di-gitignore): `.env`, `vendor/`, `node_modules/`, `public/build/`. Maka setelah clone, Anda harus menjalankan langkah-langkah berikut agar aplikasi berjalan.
...
### Langkah 4 — Atur Konfigurasi Database di `.env`

Buka file `.env` dan sesuaikan koneksi database SQL Server Anda:
...
> Jika belum yakin dengan konfigurasi, lihat contoh lengkap di [.env.example](.env.example).
...
Perintah ini membuat tabel database **dan** mengisi data awal (groups, users, menus, menu access, system settings).

### Langkah 6 — Instal Dependensi Frontend (npm)

```bash
npm install
```

### Langkah 7 — Build Frontend

```bash
npm run build
```

> Aplikasi menggunakan **build production** (bukan dev server). File `public/build/` di-gitignore, sehingga wajib di-build setelah clone.
...
Lalu akses: `http://localhost:8000/tdk-core-pkl`

Atau jika menggunakan Apache, arahkan document root ke folder `public/` (contoh: `http://localhost:83/tdk-core-pkl`).
...
| Intern | `dapina` | `dapina123` |
| Intern | `maura` | `maura123` |
...
└── public/build/                  # Hasil vite build (di-gitignore)
```
